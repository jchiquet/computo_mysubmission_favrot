...
```{r figure1, cache = TRUE, fig.width=12, fig.height=14}
#| echo: false
#| label: fig-one 
...
```{r inference_example_real_data, cache = TRUE, results = 'hide'}
#| echo: true
#| eval: false
...
::: {.callout-note}
The following code presents the inference for one seed. As the computation time is growing fast with the number of trials and the number of sampled values, we set here the number of trials to 10 (I = 10) and the number of adaptations and iterations to 2000.

Our simulation results are obtained by running this code for I = 10, 20, 40, 80, for all seeds between 0 and 999, with a number of adaptations and iterations equal to 36000. We used the Migale server from INRAE to run these simulations.
...
```{r figure2, cache = TRUE, out.width="100%", fig.height = 7, echo = FALSE}
#| label: fig-two
...
```{r figure3, cache = TRUE, out.width="100%", fig.height = 7, echo = FALSE}
#| label: fig-three
...
tab4 %>% 
  kbl(booktabs = TRUE, caption = "Summary of the posterior distributions obtained with the 100 Y scenario for the model parameters and treatment efficacies: posterior mean, standard deviation, 2.5 and 97.5 quantiles.") %>%
  kable_styling()
```
...
```{r figure4, cache = TRUE, echo=FALSE, out.width = "100%", fig.height = 2.6}
#| label: fig-four
...
```{r load_res_simulations, cache = TRUE, echo = FALSE}
...
```{r figure5, cache = TRUE, out.width="100%", fig.height = 2.5, echo = FALSE}
#| label: fig-five
...
```{r figure7, cache = TRUE, out.width="100%", fig.height = 2.5, echo = FALSE}
#| label: fig-seven
...
```{r figure8, cache = TRUE, out.width="100%", fig.height = 6, echo = FALSE}
#| label: fig-height
