...
```{r dependancies, cache = FALSE, include = FALSE}
library(ggplot2);   library(tidyverse);   library(gridExtra);   library(rjags);   library(paletteer);   
library(latex2exp);   library(grid);   library(dplyr);   library(tidyr);   library(tibble)
...
col = c(paletteer_d("awtools::spalette", n = 5))[- c(2)]
